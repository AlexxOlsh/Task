...
class TaskTestCase(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(email='test@test.ru', password='testpassword')
    ...
    def test_update_task(self):
        self.task = Task.objects.create(name="Test Task", description="Test Description", owner=self.user)
        response = self.client.put(reverse('todolist:task-update', kwargs={'pk': self.task.pk}), data={
            'name': 'Replacement 1',
            'description': 'Replacement 2'})
        self.assertEqual(response.status_code, 200)
        task = Task.objects.first()
        self.assertEqual(task.name, 'Replacement 1')
...
class CommentTestCase(TestCase):
    ...
    def test_create_comment(self):
        response = self.client.post(reverse('todolist:comment-create'),
                                    data={'name': self.task.pk, 'comment': 'Test Comment'})
        self.assertEqual(response.status_code, 201)
        comment = Comment.objects.first()
        self.assertEqual(comment.comment, 'Test Comment')
        self.assertEqual(comment.name, self.task)

    def test_view_comment(self):
        comment = Comment.objects.create(name=self.task, comment='Test Comment')
        ...
        self.assertEqual(retrieved_comment.comment, 'Test Comment')
        self.assertEqual(retrieved_comment.name, self.task)
